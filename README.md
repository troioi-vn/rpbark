...
Right now this repository is the game's scaffold: a small playable foundation with movement, stamina-backed running, jumping, visual barking, parallax background layers, dog sprite state switching, and a tiny pause/save/load menu. The goal is to keep that base simple, readable, and easy to extend into a real RPG.
...
- current gameplay: walk, stamina-backed run, jump, bark a visual sound wave, collide with terrain, view parallax scenery, save/load player position and stamina from the Esc menu
